...
<template>
    ...
    <body>
        <h1 v-html="$t('pages.' + currentRouteName + '.header')"></h1>
...
const products = [
    { name: 'Varnish', description: 'Scan site', href: '/varnish/', icon: ChartPieIcon },
...
export default {
    ...
    computed: {
        currentRouteName() {
            return this.getCurRoute().name;
        },
        currentRouteIcon() {

            return this.getCurRoute().icon;
        }
    },
    ...
    methods: {
        getCurRoute() {
            ...
            if(typeof (rt) === 'object') {return rt;} else {return {icon:'fas fa-home', title: 'Главная'}}
        }
    },

    watch: {
        $route(to, from) {
            this.history = from.fullPath
        }
    },
